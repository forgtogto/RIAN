...
public class DatabaseHandler {
	...
	public int connect() {
		try {
			Class.forName(Setting.DASEBASE_DRIVER);
			try {
				connection = DriverManager.getConnection(
						"jdbc:mysql://localhost:" + PORT + "/" + Setting.DEFAULT_USE_DATABASE,
						Setting.DEFAULT_DATABASE_HOST_ID, Setting.DEFAULT_DATABASE_HOST_PASSWORD);
	...
	public ResultSet excuteQuery(String query) {
		/*
		 * 여기서 쿼리문 통해서 실제 디비에 전달된 addValue를 디비에 삽입시켜야함
		 */
		ResultSet rs = null;
		try {
